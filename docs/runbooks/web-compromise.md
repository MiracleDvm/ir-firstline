...
1. **Confirm the compromise.** Check for defaced content, an unexpected file in the web root, unrecognized processes or connections, or the alert that triggered this (WAF/IDS rule, file-integrity check).
   ...
   - **CLI / open-source alternative**: compare the current page/files against a known-good baseline by hand; list recently modified files with `find /var/www -mtime -1 -type f` (Linux) or `Get-ChildItem -Recurse | Sort-Object LastWriteTime -Descending` (Windows).
...
5. **Disable accounts and credentials that may be compromised** — especially any local or admin account on the server that you don't recognize.
   ...
   - **CLI / open-source alternative**: `usermod -L <account>` (Linux) or `net user <account> /active:no` (Windows); review `/etc/passwd` for unexpected UID 0 entries.
...
- Tools cited as examples: YARA, Sysinternals Autoruns, AIDE, rkhunter, Sleuth Kit/Autopsy, Volatility, FTK Imager, HTTrack, TheHive.
